...
int main (void) {
  ...
  /* Put main program loop code here */
		...
		if (tempChar > '0' && tempChar < '6') {
			PutChar(tempChar);
			TPM0->CONTROLS[2].CnV = PWM_duty_table [tempChar - '1'];
			NewLine();
			UInt8 Segment = (UInt8)(tempChar - '0');
			...
			PutNumHex((UInt32) (DAC0_table [Segment - 1]));
			NewLine();
			...
			//Read digital value
			UInt16 ADCVal = ADC0->R[0];
			
			PutStringSB("New digital value: 0x", MAX_STRING);
			PutNumHex((UInt32) ADCVal);
			NewLine();
			
			PutStringSB("\"Analog\" servo position: ", MAX_STRING);
			int newVal = ((ADCVal*5)/1024) + 1;
			PutChar(newVal + '0');
			NewLine();
			
			UInt8 ServoPosition;
			ServoPosition = Segment; /* Desired servo position */
			//TPM0->CONTROLS[2].CnV = PWM_duty_table [ServoPosition - 1];
			PutNumHex(ServoPosition);
